...
const JobCard: React.FC<JobCardProps> = ({ job, onViewApplications }) => {
  ...
  return (
    <div className="bg-white   rounded-xl p-6  transition-all duration-300 group h-fit">
      ...
      <button
        ...
        className="w-full bg-brand hover:bg-brand/90 text-white rounded-full py-3"
        disabled={job.application_count === 0}
      >
        {job.application_count === 0 ? 'No Applications' : 'View Applications'}
      </button>
