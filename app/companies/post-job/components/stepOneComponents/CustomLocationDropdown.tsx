...
const CustomLocationDropdown: React.FC<CustomLocationDropdownProps> = ({ 
  ...
  // Transform the job_location_types object from API into the format needed for the dropdown
  const locationTypes = React.useMemo(() => {
    // If we have job location types from the API, use them
    if (jobOptions?.job_location_types) {
      const apiLocationTypes = Object.entries(jobOptions.job_location_types).map(([key, value]) => ({
        value: key,
        label: String(value)
      }));
  ...
  const getDisplayText = () => {
    const selected = locationTypes.find(type => type.value === selectedValue);
    ...
    <div className={`mb-4 ${className}`}>
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        <button
          ...
          `}
          disabled={!!error || locationTypes.length <= 1}
        >
          <div className="flex items-center justify-between">
            <span>
              {error ? "Error loading location types" : 
               locationTypes.length <= 1 ? "No location types available" : 
               getDisplayText()}
            </span>
            {!error && locationTypes.length > 1 && (
              <ChevronDown 
        ...
        {/* Dropdown Options */}
        {isOpen && !error && locationTypes.length > 1 && (
      ...
      {error && (
        <p className="mt-1 text-sm text-red-600">Error loading location types</p>
      )}
    </div>
  );
};
