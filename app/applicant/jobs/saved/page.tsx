...
import { useSavedJobs } from '@/hooks/jobs/useSaved';
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <p className="text-gray-600 mb-4">Something went wrong while fetching your saved jobs.</p>
        <button 
          onClick={() => refetch()}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className='p-2 md:p-0'>
          ...
          <p className="text-gray-600 mt-1">
            {totalCount} {totalCount === 1 ? 'job' : 'jobs'} saved
          </p>
        </div>
      </div>
      ...
      {!isLoading && (
        <div className='rounded-xl my-6'>
          {filteredAndSortedJobs.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12">
              <Icon icon="heroicons:bookmark" className="w-16 h-16 text-gray-300 mb-4" />
              <h3 className="text-lg font-semibold text-gray-900 mb-2">
                {searchQuery ? 'No jobs found' : 'No saved jobs yet'}
              </h3>
              <p className="text-gray-600 text-center max-w-md">
                {searchQuery 
                  ? 'Try adjusting your search terms or clearing the search to see all saved jobs.'
